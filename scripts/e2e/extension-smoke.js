...
try {
  ...
  assert.equal(health.version, '1.3.4', 'v1.3.4 watchdog must be running');
...
async function projectConsoleSuite() {
  ...
  await waitEval(pageA, "document.getElementById('chatsentinel-project-console-host')?.shadowRoot?.getElementById('footerVersion')?.textContent.includes('v1.3.4')");
  ...
  await waitEval(setupTarget, "document.getElementById('version')?.textContent.includes('v1.3.4')");
