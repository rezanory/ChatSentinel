...
const server = http.createServer((req, res) => {
  ...
  const kind = url.pathname.slice(1) || 'idle';
...
function render(kind, id) {
  ...
  return `<!doctype html><html data-chatsentinel-conversation-id="${id}"${progress}>
    <head><title>${kind}</title></head><body>${state}${composer}</body></html>`;
}
